...
import { Scale, Calendar, Search, ArrowRight, User, CalendarDays, Monitor, PlayCircle, Info } from "lucide-react"
  ...
  const getFilteredHearings = (): PublicHearingItem[] => {
    ...
    if (activeTab === 'hari_ini') {
      filtered = filtered.filter(
        (h) => h.tanggal_sidang === todayStr && h.status_sidang !== 'Selesai'
      )
      ...
      filtered = filtered.filter(
        (h) => h.status_sidang === 'Selesai' || h.tanggal_sidang < todayStr
      )
    }
    ...
    <div className="min-h-screen bg-white font-sans text-slate-900 selection:bg-blue-100 selection:text-blue-900">
      ...
      <nav className="fixed top-0 inset-x-0 z-50 bg-white/80 backdrop-blur-md border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <Link
              to="/login"
              className="hidden sm:inline-flex items-center justify-center px-4 py-2 text-sm font-medium text-slate-700 bg-slate-50 hover:bg-slate-100 border border-slate-200 rounded-md transition-colors"
            >
              Log in
            </Link>
            <Link
      ...
      <section className="pt-32 pb-20 md:pt-40 md:pb-28 px-6 text-center max-w-5xl mx-auto">
        ...
        <p className="text-lg md:text-xl text-slate-600 max-w-3xl mx-auto mb-10 leading-relaxed">
          {data?.pengadilan_nama ?? 'Pengadilan Tinggi'} menyediakan akses mudah bagi masyarakat
          untuk melihat jadwal sidang terbuka, memantau agenda perkara, dan menonton siaran langsung
          persidangan tanpa harus hadir ke ruang sidang.
        </p>

        <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
          <a
            href="#jadwal"
            className="w-full sm:w-auto inline-flex items-center justify-center px-8 py-3.5 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-md shadow-sm transition-all"
          >
            Lihat Jadwal Sidang
          </a>
          <a
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-10 gap-6">
          <div className="max-w-2xl">
            ...
            <p className="text-slate-600">
              Temukan informasi persidangan berdasarkan waktu atau cari langsung menggunakan nomor
              perkara dan nama terdakwa.
            </p>
          </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredHearings.map((h) => (
              <div
                key={h.id}
                className="group relative bg-white border border-slate-200 rounded-xl p-6 shadow-sm hover:shadow-md transition-all hover:border-slate-300 flex flex-col"
              >
                {/* Header Card */}
                ...
                <h3
                  className="text-lg font-bold text-slate-900 mb-4 line-clamp-2"
                  title={h.nomor_perkara}
                >
                  {h.nomor_perkara}
                </h3>
                ...
                <div className="space-y-3 mb-6 flex-1">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-semibold text-slate-800 mt-0.5">
                        {new Date(h.tanggal_sidang).toLocaleDateString('id-ID', {
                          day: 'numeric',
                          month: 'long',
                          year: 'numeric',
                        })}
        ...
        <div className="grid md:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <p className="text-slate-600 mb-8 leading-relaxed">
              Kami berkomitmen untuk menyediakan informasi persidangan secara real-time yang dapat
              diakses oleh siapa saja, di mana saja, tanpa perlu melakukan registrasi akun.
            </p>
          ...
          <div className="bg-slate-50 rounded-2xl p-8 border border-slate-200 flex flex-col justify-center min-h-[300px]">
            ...
            <p className="text-sm text-slate-600 mb-6">
              Silakan hubungi helpdesk atau kunjungi portal resmi kami untuk informasi lebih lanjut
              mengenai tata cara persidangan elektronik.
            </p>
            <a
              href="#"
              className="inline-flex items-center justify-center px-4 py-2 text-sm font-medium text-white bg-slate-900 hover:bg-slate-800 rounded-md transition-colors w-fit"
            >
      ...
      <footer className="border-t border-slate-200 bg-white py-8">
        <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <p className="text-sm text-slate-500">
            &copy; {new Date().getFullYear()} {data?.pengadilan_nama ?? 'Pengadilan Tinggi'}. All
            rights reserved.
          </p>
