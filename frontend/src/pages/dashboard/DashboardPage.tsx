...
export default function DashboardPage() {
      ...
      <div className="p-8 max-w-7xl mx-auto font-sans">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="divide-y divide-slate-100">
              {recentAudits.length === 0 ? (
                ...
                recentAudits.map((log) => (
                  <div
                    key={log.id}
                    className="p-4 px-6 hover:bg-slate-50 transition-colors flex items-start gap-4"
                  >
          ...
          <div className="bg-slate-900 rounded-xl border border-slate-800 shadow-sm p-6 text-white h-fit sticky top-6">
            <h3 className="font-bold text-lg mb-6 border-b border-slate-700 pb-3">
              Akses Cepat Tata Kelola
            </h3>
            <div className="space-y-3">
              <button
                onClick={() => navigate('/users')}
                className="w-full flex items-center justify-between p-3 bg-slate-800 hover:bg-slate-700 rounded-lg transition-colors border border-slate-700"
              >
            ...
            <div className="mt-8 pt-6 border-t border-slate-700">
              <p className="text-xs text-slate-400 leading-relaxed">
                Anda masuk sebagai Administrator. Setiap perubahan konfigurasi dan hak akses dicatat
                dalam audit log permanen.
              </p>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto font-sans">
      ...
      {loading && !data && (
        <div className="text-slate-500 mb-4 animate-pulse">Memuat ringkasan kerja...</div>
      )}
      {error && (
        <div className="p-4 mb-6 bg-red-50 text-red-700 rounded-xl border border-red-200">
          {error}
      ...
      {data && (
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-8">
            ...
            <div
              className={`bg-white rounded-xl border ${data.peserta_menunggu > 0 ? 'border-orange-300 ring-2 ring-orange-100' : 'border-slate-200'} p-6 shadow-sm transition-all`}
            >
              <div className="flex items-center gap-3 mb-3">
                <div
                  className={`p-2 rounded-lg ${data.peserta_menunggu > 0 ? 'bg-orange-100 text-orange-600' : 'bg-slate-50 text-slate-400'}`}
                >
                  <Users size={20} />
                </div>
                <span className="text-sm font-bold text-slate-600 uppercase tracking-wide">
              ...
              <div className="flex items-end gap-3">
                <p
                  className={`text-4xl font-extrabold ${data.peserta_menunggu > 0 ? 'text-orange-600' : 'text-slate-900'}`}
                >
              ...
              {data.sidang_list.length === 0 ? (
                <div className="flex-1 p-12 text-center flex flex-col items-center justify-center">
                  ...
                  <p className="text-slate-500 text-sm mt-1 max-w-sm">
                    Tidak ada agenda persidangan yang dijadwalkan untuk sisa hari ini.
                  </p>
                      ...
                      <div className="flex items-start gap-4">
                        <div className="w-16 shrink-0 pt-1 text-center">
                          <p className="text-sm font-bold text-slate-800">
                            {s.jam_sidang.slice(0, 5)}
                          </p>
                          <p className="text-[10px] font-bold text-slate-400 uppercase">WIB</p>
                        </div>
                        <div className="border-l-2 border-blue-200 pl-4 flex-1">
                          <h4
                            className="font-bold text-blue-900 text-lg group-hover:text-blue-700 cursor-pointer transition-colors"
                          ...
                          <div className="flex flex-wrap items-center gap-2 mt-1.5 mb-3">
                            ...
                            <span
                              className={`text-xs px-2 py-0.5 rounded-full font-bold ${s.status_transparansi === 'open' ? 'bg-blue-50 text-blue-700' : 'bg-red-50 text-red-700'}`}
                            >
                          ...
                          <div className="flex items-center gap-2">
                            <button
                              ...
                              className="text-xs font-semibold text-slate-600 hover:text-blue-700 bg-white border border-slate-200 hover:border-blue-300 px-3 py-1.5 rounded shadow-sm transition-all flex items-center gap-1.5"
                            >
                              <Users size={14} /> Waiting Room
                            </button>
                            <button
                              onClick={() => navigate(`/hearings/${s.id}`)}
                              className="text-xs font-semibold text-blue-700 hover:text-blue-800 hover:underline px-2 py-1.5"
                            >
                              Lihat Detail &rarr;
                            </button>
                          </div>
                ...
                {tasks.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center p-8 text-center text-slate-400">
                    ...
                    <p className="text-xs mt-1">
                      Tambahkan tugas baru untuk mulai melacak pekerjaan Anda hari ini.
                    </p>
                  </div>
                ) : (
                  <ul className="divide-y divide-slate-100">
                    {tasks.map((t) => (
                      <li
                        key={t.id}
                        className="p-4 bg-white hover:bg-slate-50 flex items-start gap-3 group transition-colors"
                      >
                        <button
                        ...
                        <div className="flex-1 min-w-0">
                          <p
                            className={`text-sm font-medium truncate ${t.status === 'completed' ? 'text-slate-400 line-through' : 'text-slate-800'}`}
                          >
                            {t.title}
                          </p>
