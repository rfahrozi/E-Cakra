...
export default function HearingCreatePage() {
  ...
  // State untuk dropdown dinamis dari DB
  ...
  useEffect(() => {
    settingsApi.getGeneral()
      ...
      .catch(err => console.error("Gagal load opsi dropdown", err))
  }, [])
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      const hearing = await hearingsApi.create(finalData)
      setHearingId(hearing.id)
      const tmpl = await hearingsApi.template(hearing.id)
      setTemplate(tmpl)
    } catch (err: unknown) {
      const e = err as { response?: { data?: { detail?: string } } }; const detail = e.response?.data?.detail
      setError(
        typeof detail === 'string' ? detail : 'Gagal membuat sidang. Periksa konfigurasi Zoom.'
      )
    } finally {
      setLoading(false)
    }
    ...
    <div className="p-8 max-w-4xl">
      <div className="mb-6">
        ...
        <p className="text-gray-500 text-sm mt-1">
          Isi data sidang dan sistem akan membuat Zoom meeting secara otomatis
        </p>
      </div>
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            {template && (
              ...
              >
                {copied ? (
                  <>
                    <CheckCheck size={15} className="text-green-600" /> Tersalin!
                  </>
                ) : (
                  <>
